class AudioWebSocketClient {
  ...
  handleMessage(data) {
    ...
    if (eventType === 'audio') {
      ...
      console.log('[HANDLE] Session ended event received');
  ...
  showDownloadLink(audioUrl) {
    ...
    if (this.wsUrl) {
      try {
        ...
        console.error('[DOWNLOAD] Failed to parse WebSocket URL:', error);
  ...
  disconnect() {
    ...
    if (this.ws && this.ws.readyState === WebSocket.OPEN) {
      ...
      try {
        ...
        // Wait briefly for session.ended before forcing close
